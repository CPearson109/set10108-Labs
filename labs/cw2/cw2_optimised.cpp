...
__global__ void calculateColorTemperatureKernel(const rgba_t* rgbaData, double* output, int size)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;

    rgba_t rgba = rgbaData[idx];
...
// GPU function to calculate color temperature
std::vector<double> calculateColorTemperatureGPU(const std::vector<rgba_t>& rgbaData)
{
    int size = rgbaData.size();
    size_t dataSize = size * sizeof(rgba_t);
    size_t outputSize = size * sizeof(double);

    rgba_t* d_rgbaData;
    double* d_output;

    cudaMalloc(&d_rgbaData, dataSize);
    cudaMalloc(&d_output, outputSize);

    cudaMemcpy(d_rgbaData, rgbaData.data(), dataSize, cudaMemcpyHostToDevice);

    int threadsPerBlock = 256;
    int blocksPerGrid = (size + threadsPerBlock - 1) / threadsPerBlock;

    calculateColorTemperatureKernel << <blocksPerGrid, threadsPerBlock >> > (d_rgbaData, d_output, size);

    std::vector<double> temperatures(size);
    cudaMemcpy(temperatures.data(), d_output, outputSize, cudaMemcpyDeviceToHost);
...
double filename_to_median(const std::string& filename)
{
    ...
    auto temperatures = calculateColorTemperatureGPU(rgbadata);
...
int main()
{
    ...
    std::thread sortingThread([&]() {
        ...
        // Compute median color temperatures in parallel using std::async
        ...
        // Asynchronously copy and rename the images into the sorted folder
        ...
        for (const auto& pair : filenameMedianPairs)
        {
            ...
            copyFutures.emplace_back(std::async(std::launch::async, [sourceFile, destinationFile]() {
                try
                {
